...
public class UIInventoryBar : MonoBehaviour
{
        ...
        if(inventorySlot.Length > 0)
        {
            ...
            for (int i = 0; i < inventorySlot.Length; i++)
            {
                if(inventorySlot[i].isSelected)
                {
                    ...
                    //Update inventory to show items as not selected.
            ...
            if (inventorySlot.Length > 0 && inventoryList.Count > 0)
            {
                ...
                for (int i = 0; i < inventorySlot.Length; i++)
                {
                    if(i < inventoryList.Count)
                    {
                        ...
                        //Item Details itemDetails = InventoryManager.Instance.itemList.itemDetails.Find(x => x.itemCode == itemCode);
    ...
    private void SwitchInventoryBarPosition()
    {
        ...
        {
            //transform.position = new Vector3(transform.position.x, mainCamera.pixelHeight = 120f, 0f); //this was changed to control the recctransform see below
